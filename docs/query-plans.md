...
Captured locally on a handful of rows. The planner will not pick the same path at 1M posts. What matters is which index each query can use, and what to look for once the table is large.
...
This is the partial `(created_at DESC) WHERE deleted_at IS NULL` index. At 1M kept rows the plan to want is still this Index Scan plus a short Limit, not a sort of the whole table.
...
On a tiny table Postgres prefers the created_at partial and filters.
...
Same small-table story: the GIN on `metadata` exists (`jsonb_path_ops`, sized for `@>`) and the planner ignored it because a sequential-ish index walk is cheaper at n=4. At real volume, look for a Bitmap Index Scan on `index_posts_on_metadata`.
...
Point update by primary key. Incremental stats stay O(1) per write. A plan that `SUM`s the ratings table here would mean the incremental path regressed.
